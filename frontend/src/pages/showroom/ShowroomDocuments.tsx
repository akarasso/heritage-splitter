...
import type { DocumentInfo, PublicUser, ShowroomParticipantDetail } from "~/lib/api-client";
...
export default function ShowroomDocuments() {
  const { showroom, user, isOwner, isMember } = useShowroom();

  const [documents, { refetch }] = createResource(
    () => showroom()?.id,
    (id) => api.listShowroomDocuments(id)
  );
  ...
  async function handleUpload(file: File) {
    ...
    setUploading(true);
    try {
      await api.uploadShowroomDocument(sr.id, file);
      refetch();
    } catch (e) {
  ...
  async function handleCertify(doc: DocumentInfo) {
    ...
    try {
      ...
      if (wallets.length === 0) throw new Error("Connect your wallet to certify a document");
      ...
      const signature = await walletClient.signTypedData({
        account: address,
        domain: { name: "DocumentRegistry", version: "1", chainId: BigInt(appChain.id), verifyingContract: registryAddress },
        types: { Certify: [{ name: "hash", type: "bytes32" }, { name: "certifier", type: "address" }, { name: "nonce", type: "uint256" }, { name: "deadline", type: "uint256" }] },
        primaryType: "Certify",
        message: { hash: hashHex, certifier: address, nonce: BigInt(nonce), deadline: BigInt(deadline) },
      });
  ...
  const acceptedParticipants = () =>
    (showroom()?.participants || []).filter(
      (p: ShowroomParticipantDetail) => p.status === "accepted" && p.user_id !== user()?.id
    );
      ...
      fallback={
        <div class="text-center py-20">
          ...
          <p class="text-sm" style={{ color: "var(--text-muted)" }}>
            Only showroom members can view documents.
          </p>
        </div>
      }
    >
      <div class="space-y-8 animate-fade-in-up">
        ...
        <Show when={isOwner()}>
          <div
            ...
            style={{
              border: dragOver() ? "2px dashed var(--gold)" : "2px dashed var(--border)",
              background: dragOver() ? "rgba(212,168,83,0.05)" : undefined,
            }}
            onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
            onDragLeave={() => setDragOver(false)}
            onDrop={onDrop}
            onClick={() => document.getElementById("showroom-doc-upload-input")?.click()}
          >
            <div class="flex flex-col items-center justify-center py-8 gap-3">
              <svg class="w-10 h-10" style={{ color: "var(--text-muted)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.233-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z" />
              </svg>
              <p class="text-sm" style={{ color: "var(--cream-muted)" }}>
                {uploading() ? "Uploading..." : "Drag a file here or click to upload"}
              </p>
              <p class="text-xs" style={{ color: "var(--text-muted)" }}>
                Max 50 MB — Automatic AES-256 encryption
              </p>
            </div>
            <input id="showroom-doc-upload-input" type="file" class="hidden" onChange={onFileInput} />
          </div>
        </Show>
          ...
          >
            ...
            >
              <div class="space-y-2">
                <For each={documents()!}>
                  {(doc) => (
                    <div class="p-4 rounded-lg relative" style={{ background: "var(--noir-light)" }}>
                      {/* Certifying overlay */}
                      <Show when={certifying() === doc.id}>
                        <div class="absolute inset-0 rounded-lg flex items-center justify-center gap-3 z-10" style={{ background: "rgba(18,18,20,0.85)" }}>
                          <div class="w-5 h-5 rounded-full border-2 border-t-transparent animate-spin" style={{ "border-color": "var(--gold)", "border-top-color": "transparent" }} />
                          <span class="text-sm font-medium" style={{ color: "var(--gold)" }}>Certifying...</span>
                        </div>
                      </Show>
                      <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3 min-w-0 flex-1">
                          <div class="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: "var(--surface-light)", color: "var(--gold)" }}>
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                            </svg>
                          </div>
                          <div class="min-w-0">
                            <p class="text-sm font-medium truncate" style={{ color: "var(--cream)" }}>{doc.original_name}</p>
                            <div class="flex items-center gap-2 mt-0.5">
                              <span class="text-xs" style={{ color: "var(--text-muted)" }}>{formatSize(doc.file_size)}</span>
                              <span class="text-xs" style={{ color: "var(--text-muted)" }}>{formatDate(doc.created_at)}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                          <Show when={doc.certified_at}>
                            {doc.tx_hash ? (
                              <a
                                ...
                                style={{ color: "var(--emerald)", background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.3)" }}
                                onClick={(e) => e.stopPropagation()}
                              >
                                Certified
                              </a>
                            ) : (
                              <span class="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ color: "var(--emerald)", background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.3)" }}>
                                Certified
                              </span>
                            )}
                          </Show>
                          ...
                          <button
                            ...
                            style={{ background: "var(--surface-light)", color: "var(--cream-muted)" }}
                            onClick={() => handleDownload(doc)}
                          >
                            Download
                          </button>

                          <Show when={isOwner() && !doc.certified_at}>
                            <button
                              class="text-xs px-3 py-1.5 rounded-lg font-medium transition-all"
                              style={{ background: "rgba(212,168,83,0.12)", color: "var(--gold)", border: "1px solid rgba(212,168,83,0.3)" }}
                          ...
                          <Show when={isOwner()}>
                            <button
                              class="text-xs px-3 py-1.5 rounded-lg font-medium transition-all"
                              style={{ background: "rgba(167,139,250,0.12)", color: "var(--violet)", border: "1px solid rgba(167,139,250,0.3)" }}
                              onClick={() => { setShareDocId(doc.id); setSelectedUsers([]); }}
                            >
                              Share
                            </button>
                          </Show>
                      ...
                      {/* Blockchain info */}
                      <Show when={doc.certified_at}>
                        <div class="mt-3 pt-3 space-y-1.5" style={{ "border-top": "1px solid var(--border)" }}>
                          <div class="flex items-center gap-2">
                            ...
                            <code
                              ...
                              onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(doc.sha256_hash); }}
                            >
          ...
          >
            <div class="card w-full max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <div class="space-y-2 mb-4 max-h-60 overflow-y-auto">
                <For each={acceptedParticipants()}>
                  {(pt) => (
                    <label
                      class="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all"
                      style={{
                        background: selectedUsers().includes(pt.user_id) ? "rgba(167,139,250,0.08)" : "var(--noir-light)",
                        border: selectedUsers().includes(pt.user_id) ? "1px solid rgba(167,139,250,0.3)" : "1px solid transparent",
                      }}
                    >
                      <input
                        type="checkbox"
                ...
                <Show when={acceptedParticipants().length === 0}>
                  <p class="text-xs py-4 text-center" style={{ color: "var(--text-muted)" }}>
                    No accepted participants to share with.
                  </p>
                </Show>
              </div>
              <div class="flex gap-3">
                ...
                <button class="btn-secondary text-sm" onClick={() => setShareDocId(null)}>
                  Cancel
                </button>
              </div>
            </div>
